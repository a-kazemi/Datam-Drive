      ...
      >
        <Section title="General">
          <Field
            ...
            hint="Launch DatamDrive automatically when you sign in."
          >
            <label
              style={{
                display: "flex",
        ...
        <Section title="Sync">
          <Field
            label="Poll interval"
            hint="How often to check SharePoint for changes (10–300 seconds)"
          >
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <input
                ...
                style={{ flex: 1 }}
              />
              <span style={{ fontSize: 13, color: "#555", width: 60 }}>
                {pollSecs}s
              </span>
            </div>
          </Field>
          ...
          <Field
            label="Max file size"
            hint="Files larger than this are skipped with an error notification"
          >
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <input
                type="range"
        ...
        <Section title="Updates">
          <Field
            label="Auto-update"
            hint="Check for updates on startup and notify when a new version is available. Off by default — required for air-gapped deployments."
          >
            <label
              style={{
                display: "flex",
          ...
          {settings.autoUpdate && (
            <Field
              ...
              hint="Leave blank to use GitHub Releases. For air-gapped deployments, point to your self-hosted server."
            >
              <input
                type="url"
                value={settings.updateFeedUrl}
                onChange={(e) =>
                  setSettings((s) => ({ ...s, updateFeedUrl: e.target.value }))
                }
                placeholder="https://github.com/datam-drive/"
                style={inputStyle}
              />
            </Field>
          )}
