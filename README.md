...
DatamDrive is early-stage software. The repository currently contains the Electron desktop shell, setup flow, tray integration, SQLite-backed local state, SharePoint client scaffolding, polling/watch modules, settings, logging, and tests for selected sync/auth behaviors.
...
- Local library registration and status dashboard.
- Background sync status, activity log, pause/resume controls, and settings UI.
...
The app stores runtime state under Electron's `userData` directory. Logs are written under a DatamDrive-specific logs folder inside that location.
